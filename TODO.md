...
- [x] *Zalantza-hitzak* (`ebe-zal.txt`) — **batch 1 EGINDA (2026-08-26)**: 628 hitz bakuneko ordezkapen determinista (`abots→ahots`, `aborto→abortu`, `amapola→mitxoleta`). EBE PDFetik erauzita kolore-analisiz (RED=zaharkitua, BOLD=estandarra); ikus `RESEARCH.md` §7.12. **Ez 30-50, 628 sarrera** (hitz bakuneko garbiak; multi-hitza batch 2a-ra atzeratuta). Fitxategiak: `src/core/data/zalantza.js` (datuak, orain 739 sarrera batch 2a-rekin), `src/core/rules/zalantza-words.js` (araua, kasua mantentzen du), `tests/core/zalantza.test.mjs` (49 test). Egiaztapenak: 0 idempotentzia-overlap, 0 konposatu-fragamentu-leak, `gara` aditza baztertuta
    - [ ] *Zalantza multi-hitza* — **batch 2a (ikerketa EGINDA, inplementazio partziala)**: ikus `RESEARCH.md` §7.13. Batch 1-eko `final_extract.py`-ren komak-botatze bug-ak **~116 sarrera galdu** zituen (adib. `jatsi→jaitsi`, `eskubi→eskuin`, `rugby→errugbi`). v6 extraktoreak (pdfplumber kolore-analisia + EBE sarreren egitura-gramatika) birrobagarria da (`docs/ebe-reference/extract-zalantza.py`). Datu-fitxategiak `docs/ebe-reference/`-n:
      ...
      - [x] `zalantza-proper.tsv` (35 izen berezi → **EGINDA**: 24 sarrera seguru `zalantza-proper.js`-n; 3 baztertuta: `ozkabarte` erauzketa hautsia, `donejakue`/`donibane` faltsu-positibo arriskua). F5 ikerketak erakutsi du maiuskula semantikoak testuinguru-menpekoak direla — ezin gazetteer hutsez ebatzi. Ikus `RESEARCH.md` §7.16
      - [ ] `zalantza-ambiguous.tsv` (6 anbiguo → batch 3+)
      - **Idempotentzia**: 0 kate-arrisku (X→Y→Z), 0 cross-chain batch 1-ekin, 0 aditzi-kolisio berri ✓ (validazio-scripta: `/tmp/validate_batch2a.mjs`, `/tmp/validate_proper.mjs` 24 check)
      - **Inplementazio-plana**: (1) ~~fasea: 112 sarrera `zalantza.js`-ra~~ **EGINDA** — 111 gehituta (`gara` baztertuta), gero 19 marratxodun birkalifikatuta → 720 final; (2) ~~fasea: `zalantza-phrases.js` arau berria~~ **EGINDA** — priority 46, 52 sarrera (33 TSV + 19 birkalifikatuta), 39 test; (3) ~~fasea: 35 izen berezi~~ **EGINDA** — 24 seguru `zalantza-proper.js`-n (priority 49/50, `matchCaseProper()`), 3 baztertuta, 6 anbiguo atzeratuta
    ...
    - [ ] *Kalko morfosintaktikoak* (`ebe-kal.txt` §2) — **batch 3 IKERKETA EGINDA** (`RESEARCH.md` §7.17): 44 kategoria 5 eragigaitasun-mailetan sailkatuta (A-E). **Emaitza nagusia**: 44tik 0 dira A-maila garbia; 7 B-maila (gazetteer behar), 6 C-maila (GECToR hipotesia — **FALTSUTUTA**: GECToR-ek 0/6 estaltzen ditu, transfer-erroreak ez baitira R1-R4 perturbazio sintetikoak), 13 D-maila (morfologia), 17 E-maila (testuingurua). **2 B-maila arau inplementatuta**: `gogoekin→gogoarekin`, `hobe esanda→hobeto esanda` (`calque.js`-n gehituta). Eval-estrategia eraikita: `tests/ebe-rules/cases.json` (43 kasu) + `eval.mjs` (3 modu: --rules/--gector/--check). Baseline: rules 2/43 (4.7%), rules+GECToR 1/43 (2.3%). **Ondorioa**: batch 3 ez da egingarria motor deterministarekin; D-mailarako `tokenizer.js` behar da, baina ez da lehentasun handia (idazle aurreratuentzako erroreak dira). 5 B-maila geratzen dira (arrisku ertaina: elkar, nekatuta naiz, adj posizioa, %, baino gehiago)
    - [ ] *Maiuskulak semantikoak* (F5) — **IKERKETA EGINDA** (`RESEARCH.md` §7.16): testuinguru-menpekoa da, **ezin da gazetteer hutsez ebatzi**. EBE Maiuskulak id=1023: "Donostiako Udala" (erakunde zehatza, maiuskula) vs "Gipuzkoako udaletan" (orokorra, minuskula) — hitz bera, testuinguruaren arabera. c091 (erakunde-maius), c095 (izar gorputzak), c096 (ereduak gain-maiuskulatu) benetan zail-ezagunak dira — POS/testuinguru analisia behar dute (batch 3+). c096 des-maiuskulatze arriskutsua da — atzeratu
...
- [x] **`src/core/data/calque.js`** — 6 sarrerako (3 hitz + 4 esapide) kalko hiztegia EGINDA (P1 batch 2b §1: 2 hitz + 2 esapide; batch 3 §2: 1 hitz + 2 esapide). EBE §1-eko 34 sarreretatik 8 garbiak; 4 jada zalantza.js-n (belgiar/europear/egipziar/erabakior). EBE §2-ko 44 kategoriatik 2 garbiak (gogoekin, hobe esanda). Araua: `src/core/rules/calque.js` (priority 47/48, `LintKind.Calque`)
- [x] **`src/core/data/zalantza-proper.js`** — 24 sarrerako (21 hitz bakun + 3 esapide) izendun-zalantza hiztegia EGINDA (batch 2a Phase 3). EBEren 35 izendunetatik 27 bakarrak; 3 baztertuta (`ozkabarte` hautsita, `donejakue`/`donibane` faltsu-positibo arriskua). Araua: `src/core/rules/zalantza-proper.js` (priority 49/50, `matchCaseProper()`, `LintKind.Confusable`), 46 test. Norabidea egiaztatuta EBE aurkibide analitikoa + EODA-rekin
- [x] **EBE arauen eval-estrategia** (ikus §7.11 ikerketa): Elhuyar GEC benchmark-ak (R1-R4 akats motak: aditz-denbora, kasu-kidetasuna, determinatzailea, lokailua) **ez ditu kalko/zalantza akatsik** — hauek morfologia sintetikoa dira, ez aukeraketa lexikala. (1) zalantza/kalko lexikoak unit-testen bidez ebaluatu **EGINDA** (247 test); (2) kalko morfosintaktikoak → **EGINDA**: `tests/ebe-rules/cases.json` (43 kasu, 5 maila A-E) + `tests/ebe-rules/eval.mjs` (3 modu: --rules/--gector/--check). Baseline: rules 2/43 (4.7%), rules+GECToR 1/43 (2.3%). **GECToR-ek EZ ditu C-maila kalkoak estaltzen** (0/6) — transfer-erroreak ≠ perturbazio sintetikoak. Ikus `RESEARCH.md` §7.17
...
- [ ] **(Atzeratuta) `src/core/tokenizer.js` POS-rekin** — Harper-ren Brill POS tagger ez da beharrezkoa EBE zalantza/kalko lexikoetarako. **Beharrezkoa izango da D-maila kalko morfosintaktikoetarako** (batch 3: 13 kategoria — `*Nekatuta naiz`, pasiboak, `ere` posizioa, erlatiboak). §7.17-k erakutsi du 44 EBE §2 kategoriatik 13k morfologia behar dutela. Baina ez da lehentasuna — idazle aurreratuentzako erroreak dira. Euskal FST (IXA Morfeus/Foma) existatzen da baina erabilgarritasuna argi ez dagoena. Bide arinagoa: atzizki-zerrenda (-ik partitiboa, -z instrumental)
