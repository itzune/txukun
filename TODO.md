...
- [x] *Zalantza-hitzak* (`ebe-zal.txt`) — **batch 1 EGINDA (2026-08-26)**: 628 hitz bakuneko ordezkapen determinista (`abots→ahots`, `aborto→abortu`, `amapola→mitxoleta`). EBE PDFetik erauzita kolore-analisiz (RED=zaharkitua, BOLD=estandarra); ikus `RESEARCH.md` §7.12. **Ez 30-50, 628 sarrera** (hitz bakuneko garbiak; multi-hitza batch 2a-ra atzeratuta). Fitxategiak: `src/core/data/zalantza.js` (datuak), `src/core/rules/zalantza-words.js` (araua, kasua mantentzen du), `tests/core/zalantza.test.mjs` (30 test). Egiaztapenak: 0 idempotentzia-overlap, 0 konposatu-fragamentu-leak, `gara` aditza baztertuta
    - [ ] *Zalantza multi-hitza* — **batch 2a (ikerketa EGINDA, inplementazio atzera)**: ikus `RESEARCH.md` §7.13. Batch 1-eko `final_extract.py`-ren komak-botatze bug-ak **~116 sarrera galdu** zituen (adib. `jatsi→jaitsi`, `eskubi→eskuin`, `rugby→errugbi`). v6 extraktoreak (pdfplumber kolore-analisia + EBE sarreren egitura-gramatika) birrobagarria da (`docs/ebe-reference/extract-zalantza.py`). Datu-fitxategiak `docs/ebe-reference/`-n:
      - `zalantza-new-singles.tsv` (93 bikote → `zalantza.js`-ra gehitu, kode-aldaketarik gabe)
      - `zalantza-type-c.tsv` (19 bikote: hitza→esapidea, adib. `abioneta→hegazkin txiki` → `zalantza.js`-ra, `matchCase()`-k jada maneiatzen ditu multi-word helburuak)
      - `zalantza-phrases.tsv` (33 multi-token RED bikote → arau berria behar: `zalantza-phrases.js` + sliding-window token matching). 2 bikotek helburu trunkatua dute (`autonomi elkarte→autonomia-elkarte`, `merkatal zentro→merkataritza-zentro` — eskuz zuzenduta)
      - `zalantza-proper.tsv` (35 izen berezi → F5 gazetteer)
      - `zalantza-ambiguous.tsv` (6 anbiguo → batch 3+)
      - **Idempotentzia**: 0 kate-arrisku (X→Y→Z), 0 cross-chain batch 1-ekin, 0 aditzi-kolisio berri ✓
      - **Inplementazio-plana**: (1) fasea: 93+19=112 sarrera `zalantza.js`-ra; (2) fasea: `zalantza-phrases.js` arau berria priority 46-n (single-word-aren atzetik), `tokenize()` bidez RED esapidia tokenizatu, sliding-window match, `matchCase` lehen tokenaren kasuan; (3) fasea: 35 izen berezi F5-era, 6 anbiguo atzeratu
    - [ ] *Kalko lexiko-semantikoak* (`ebe-kal.txt` §1) — **batch 2b**: `balore→balio`, `*ideologia anitza→askotariko ideologia`, `*pena merezi→merezi`, `*ospatu bilera→bilera egin`. Hitz bakunekoak errazak; esaldi-mailakoak ("pena merezi") frase-detekzioa behar dute (zalantza-phrases araua erabil dezake batch 2a-rekin)
...
- [x] **`src/core/data/zalantza.js`** — 628 sarrerako zalantza-hiztegia EGINDA (P1 batch 1). Izen berezien gazetteer-a (F5) oraindik atzeratuta dago — `src/core/dictionary.js` izen berezietarako sortu F5-rekin batera
