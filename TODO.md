...
- [x] *Zalantza-hitzak* (`ebe-zal.txt`) — **batch 1 EGINDA (2026-08-26)**: 628 hitz bakuneko ordezkapen determinista (`abots→ahots`, `aborto→abortu`, `amapola→mitxoleta`). EBE PDFetik erauzita kolore-analisiz (RED=zaharkitua, BOLD=estandarra); ikus `RESEARCH.md` §7.12. **Ez 30-50, 628 sarrera** (hitz bakuneko garbiak; 108 esaldi-mailako batch 2-ra atzeratuta). Fitxategiak: `src/core/data/zalantza.js` (datuak), `src/core/rules/zalantza-words.js` (araua, kasua mantentzen du), `tests/core/zalantza.test.mjs` (30 test). Egiaztapenak: 0 idempotentzia-overlap, 0 konposatu-fragamentu-leak, `gara` aditza baztertuta
    - [ ] *Zalantza esaldi-mailakoak* — **batch 2**: 108 esaldi/palismoa (adib. `asanblada asanblea→biltzar`, `asper egin→asper-asper egin`). `docs/ebe-reference/zalantza-multi.tsv`-n gordeta. Frase-detekzioa behar du (multitoken span matching). Sarrera batzuk izen bereziak dira (`Bizkaiko Golkoa→Bizkaiko golkoa`) → F5 gazetteer lerrokatuta. + 5 anbiguoak (`boltsa→burrsa|poltsa`) testuinguru-desambiguoak behar ditu
